...
if($periodgrownm=="ระยะตั้งท้อง")$periodgrownm="ระยะออกดอก";
	...
	if($row[0]!=""){	
		...
		if($num>1){
			$text0="<button id=\"btnPest".$row["id"]."\" style=\"width:".$widthBtn."px;border:solid 1px #5882FA;background-color:#5882FA;\">มีแมลง ".$num." ชนิด</button>";
			$text0.="<div id=\"Pest".$row["id"]."\" style=\"background-color:#FFF;\">";
		}
			$text0.="<table class='pestTable' style='background:nonw;width:".$widthBtn."px'>";
			$f=0;
			$text1="";
			while( $row2=mysql_fetch_array($r_loc)){
				...
				$text1.="<tr><td class='pestTD' pest_id='".$row2["pest_id"]."' colspan='2'><b>".$no.$row2[1]."</b></td></tr>";
