...
$spgm_cfg['locale']['pictureNaviBar']
...
function spgm_IsNew($strFilePath) {
  ...
  return (filemtime($strFilePath) > (time()-$spgm_cfg['conf']['newStatusDuration']*24*3600));
}
...
function spgm_DisplayThumbnailNavibar($iCurrentPageIndex, $iPageNumber, $strGalleryId, $strFilterFlags) {
  ...
  if ($iCurrentPageIndex > 1) {
    ...
    print ' &middot; '.$spgm_cfg['theme']['previousPageIconNot'];
  }
  print ' &middot; ';
  ...
  if ($iCurrentPageIndex < $iPageNumber) {
    ...
    print ' &middot; '.$spgm_cfg['theme']['lastPageIconNot'];
  }
}
  ...
  if ( spgm_IsPicture($strPictureFilename, $strGalleryId) ) {
    ...
    if ( ! $bSlideshowMode ) {
      if ($spgm_cfg['conf']['popupOverFullPictures'] == TRUE) {
        ...
        print '    <a href="#?" onclick="popupPicture(\''.$strPictureURL.'\', '.$iPopupWidth.', '.$iPopupHeight.', '.$strJustPicture.');">';
...
function spgm_DisplayThumbnails($strGalleryId, $arrPictureFilenames, $iPictureId, $iPageIndex, $strFilterFlags) {
  ...
  else $iPageIndex = ((int)($iPictureId / $spgm_cfg['conf']['thumbnailsPerPage'])) + 1;
...
;
      ...
      print '    '.spgm_BuildLink($strHtmlThumbnail, '', ANCHOR_PICTURE, $strGalleryId, -1, $i, str_replace(PARAM_VALUE_FILTER_SLIDESHOW, '', $strFilterFlags));       	  
    }    
