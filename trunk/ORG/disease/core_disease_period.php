...
$row = mysql_fetch_array($r);
	if($row[0]!=""){
		$sql_loc = "SELECT t_d.location,t_d.diseasenm, t_d.prevent ,t_d.prevent_short FROM pest_management.t_disease as t_d WHERE t_d.periodgrownm  LIKE 
			'%".$row[0]."%' and (t_d.prevent_short<>'' or t_d.prevent<>'')";
		...
		if($num>1){
			$text0="<button id=\"btnDisease".$row["id"]."\" style=\"width:".$widthBtn."px;border:solid 1px #2EFE2E;background-color:#2EFE2E;\">มีจำนวน ".$num." โรค</button>";
			...
			while( $row2=mysql_fetch_array($r_loc)){
				...
				$text1.="<tr><td colspan='2' width='".$widthBtn."px' style=\"border:solid 1px #2EFE2E;background-color:#2EFE2E;\"><b>".$no.$row2[1]."</b></td></tr>";
