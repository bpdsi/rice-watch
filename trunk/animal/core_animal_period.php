...
$row = mysql_fetch_array($r);
	if($row[0]!=""){
		...
		if($num>1){
			$text0="<button id=\"btnAnimal".$row["id"]."\" style=\"width:".$widthBtn."px;border:solid 1px #FE642E;background-color:#FE642E;\">มีสัตว์ศัตรูพืช ".$num." ชนิด</button>";
			$text0.="<div id=\"Animal".$row["id"]."\" style=\"background-color:#FFF;\">";
		}
			$text0.="<table class='animalTable' style=\"width:".$widthBtn."px;\">";
			$f=0;
			$text1="";
			while( $row2=mysql_fetch_array($r_loc)){
				...
				$text1.="<tr><td animal_id='".$row2["animal_id"]."' class='animalTD' colspan='2'><b>".$no.$row2[1]."</b></td></tr>";
